...
export const tenantsController = {
  ...
  async updateInvoiceSettings(req: Request, res: Response, next: NextFunction) {
    try {
      assertTenantAccess(req)
      const body = { ...req.body }
      if (!body.branchId && typeof req.query.branchId === 'string') {
        body.branchId = req.query.branchId
      }
      sendSuccess(res, await tenantsService.updateInvoiceSettings(req.params.id, body))
    } catch (e) { next(e) }
  },
  ...
  async getPaymentMethodSettings(req: Request, res: Response, next: NextFunction) {
  ...
  async updateRolePermissions(req: Request, res: Response, next: NextFunction) {
    try { assertTenantAccess(req); sendSuccess(res, await tenantsService.updateRolePermissions(req.params.id, req.body)) } catch (e) { next(e) }
  },
  async getMyRolePermissions(req: Request, res: Response, next: NextFunction) {
    try { sendSuccess(res, await tenantsService.getRolePermissions(req.tenantId!)) } catch (e) { next(e) }
  },
}
