...
function matchesVariation(item: PoReceiveItem, variation: Record<string, unknown>) {
  return (
    (item.sku && variation.sku === item.sku) ||
    (item.storage &&
      item.colorName &&
      variation.storage === item.storage &&
      variation.colorName === item.colorName)
  )
}
...
function applyVariationReceive(
  ...
  let updated = variations as Array<Record<string, unknown>>
  for (const { item } of group) {
    updated = updated.map((v) => {
      if (!matchesVariation(item, v)) return v
      const oldStock = Number(v.stock ?? 0)
      const oldCost = Number(v.costPrice ?? productBuyingPrice ?? 0)
      const incomingCost = item.quantity * item.unitCost
      return {
        ...v,
        stock: oldStock + item.quantity,
        costPrice: weightedBuyingPrice(oldStock, oldCost, item.quantity, incomingCost),
      }
    })
  }
  return updated
  ...
  for (const [productId, group] of byProduct) {
    ...
    await opts.tx.product.update({
      ...
      data: {
        ...
        ...(updatedVariations ? { storageVariations: updatedVariations as Prisma.InputJsonValue } : {}),
      },
    })
