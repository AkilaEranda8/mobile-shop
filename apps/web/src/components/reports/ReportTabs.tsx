...
import {
  ...
  useFinanceSummary, useDailyReloadReport, usePlStatement,
} from '@/lib/hooks'
...
export const REPORT_SECTION_TITLES: Record<ReportSectionId, { title: string; subtitle: string }> = {
  overview:    { title: 'Overview Report',      subtitle: 'KPIs · revenue trend · top products · repairs snapshot' },
  sales:       { title: 'Sales Report',         subtitle: 'Revenue trend · top selling products' },
  pl:          { title: 'P&L Report',           subtitle: 'Profit & loss statement for the selected period' },
  cashflow:    { title: 'Cash Flow Report',     subtitle: 'Inflows · outflows · net cash movement' },
  inventory:   { title: 'Inventory Report',     subtitle: 'Stock levels · low stock · category mix' },
  repairs:     { title: 'Repairs Report',       subtitle: 'Repair financial statement · status · delivered jobs' },
  delivery:    { title: 'Delivery Report',      subtitle: 'Delivery orders by status and timeline' },
  dailyreload: { title: 'Daily Reload Report',  subtitle: 'Reload sales · commission · provider breakdown' },
}
...
/* ── Single-section page shell ───────────────────────────────────── */
export function ReportSectionPage({ section }: { section: ReportSectionId }) {
  ...
  const meta = REPORT_SECTION_TITLES[section]
  ...
  return (
    <div className="space-y-5">
      ...
      {section === 'overview' && <OverviewTab days={period} fromDate={fromDate} toDate={toDate} branchId={activeBranch} />}
      {section === 'sales' && <SalesTab days={period} fromDate={fromDate} toDate={toDate} branchId={activeBranch} />}
      {section === 'pl' && <PLTab fromDate={fromDate} toDate={toDate} branchId={activeBranch} />}
      {section === 'cashflow' && <CashFlowTab fromDate={fromDate} toDate={toDate} branchId={activeBranch} />}
      {section === 'inventory' && <InventoryTab branchId={activeBranch} />}
      {section === 'repairs' && <RepairsTab branchId={activeBranch} fromDate={fromDate} toDate={toDate} />}
      {section === 'delivery' && <DeliveryTab fromDate={fromDate} toDate={toDate} branchId={activeBranch} />}
      {section === 'dailyreload' && <DailyReloadTab fromDate={fromDate} toDate={toDate} branchId={activeBranch} />}
    </div>
  )
}
