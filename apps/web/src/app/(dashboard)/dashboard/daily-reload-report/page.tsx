                ...
                <div className="space-y-3">
                  {types.map(t => (
                    <div key={t.type} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: 'var(--bg-subtle)' }}>
                      <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-brand-500/10 border border-brand-500/20">
                        {t.type === 'RECHARGE_CARD' ? <CreditCard size={14} className="text-brand-500" /> : <PhoneCall size={14} className="text-brand-500" />}
                      </div>
