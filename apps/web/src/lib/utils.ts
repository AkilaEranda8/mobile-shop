...
export function getRepairStatusColor(status: string): string {
  const colors: Record<string, string> = {
    ...
    QC: 'bg-brand-500/20 text-brand-400 border-brand-500/30',
...
export function getDeliveryStatusColor(status: string): string {
  const colors: Record<string, string> = {
    ...
    AWAITING_TRACKING: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30',
    DISPATCHED:        'bg-brand-500/20 text-brand-400 border-brand-500/30',
