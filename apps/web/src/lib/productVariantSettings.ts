...
export async function fetchProductVariantSettings(tenantId: string) {
  const { tenantApi } = await import('./api')
  const res: any = await tenantApi.getProductVariantSettings(tenantId)
  return normalizeProductVariantSettings(res?.data ?? res)
}

export async function pushProductVariantSettings(tenantId: string, settings: ProductVariantSettings) {
  const { tenantApi } = await import('./api')
  const res: any = await tenantApi.updateProductVariantSettings(tenantId, settings)
  return normalizeProductVariantSettings(res?.data ?? res)
}
